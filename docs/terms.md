# Terms of Use

Effective date: May 18, 2026

These Terms of Use apply to ComplyFlow AI.

## Service

ComplyFlow AI provides mobile tools for inspections, SOP drafting, incident reports, audit summaries, reminders, corrective action plans, reports, and operational compliance record organization.

## AI And Compliance Disclaimer

ComplyFlow AI provides operational drafting and organization support only. It does not provide legal advice, regulatory certification, safety certification, or a replacement for licensed compliance, legal, safety, or trade professionals. AI suggestions must be reviewed before use. Users remain responsible for operational compliance.

## Subscriptions

Paid digital features are offered through Apple In-App Purchase using StoreKit. Subscription options may include Pro Monthly, Pro Yearly, and Business Monthly plans. Payment is charged to your Apple Account at confirmation of purchase. Subscriptions automatically renew unless canceled according to Apple's App Store subscription settings.

You can manage or cancel subscriptions in your Apple Account settings.

## User Content

You are responsible for the accuracy, legality, review, and operational use of records, notes, photos, reports, SOPs, and other content created in the app.

## Limitations

The app is provided without guarantees that it will satisfy any specific legal, regulatory, insurance, client, audit, or safety requirement.

## Contact

Contact: `boboyen@email.com`
